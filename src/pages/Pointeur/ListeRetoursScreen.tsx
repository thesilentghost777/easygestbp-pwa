/**
 * ListeRetoursScreen.tsx
 * Liste des retours avec raisons de retour dynamiques (depuis IDB).
 */
...
import type { Produit, User as DBUser, RetourProduit } from '@/lib/db';
import {
  parseDate,
  PrixBadge,
  EditModal,
...
interface ListeRetoursScreenProps {
...
async function verifyPin(pin: string): Promise<boolean> {
  try {
    const storedPins =
      (await getConfig<Record<string, string>>('offline_pins')) || {};
...
// Couleurs par code — on garde un fallback pour les codes connus
const RAISON_COLORS: Record<string, string> = {
  perime:
    'bg-orange-100 text-orange-700 dark:bg-orange-900/30 dark:text-orange-400',
  abime: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400',
  autre: 'bg-gray-100 text-gray-700 dark:bg-gray-900/30 dark:text-gray-400',
};
const DEFAULT_RAISON_COLOR =
  'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400';

export default function ListeRetoursScreen({
  currentUser,
  retours,
  produits,
  vendeurs,
  pointeurs,
  onRefresh,
}: ListeRetoursScreenProps) {
  // Charger les raisons depuis IDB via le hook
  const raisons = useRaisons();
  ...
  const filtered = useMemo(() => {
    ...
    if (filterDate) {
      items = items.filter(r => r.date_retour?.slice(0, 10) === filterDate);
    }
    ...
    return items.sort((a, b) => {
      const da = parseDate(a.date_retour)?.getTime() ?? 0;
      const db2 = parseDate(b.date_retour)?.getTime() ?? 0;
      return db2 - da;
    });
  }, [retours, search, filterPointeur, showMyOnly, currentUser.id, produits, filterDate]);

  const openEdit = (item: RetourProduit) => {
    if (item.verrou) {
      toast.error('Cet enregistrement est verrouillé par le PDG');
      return;
    }
    setEditModal({ isOpen: true, type: 'retour', item });
    const d = parseDate(item.date_retour);
    setEditForm({
  ...
  const openDelete = (item: RetourProduit) => {
    if (item.verrou) {
      toast.error('Cet enregistrement est verrouillé par le PDG');
      return;
    }
    setDeleteModal({ isOpen: true, type: 'retour', item });
  };
  ...
  const handleSaveEdit = async (pin: string) => {
    ...
    try {
      ...
      const newDatetime = new Date(
        `${editForm.date}T${editForm.heure}:00`
      ).toISOString();
      const ret = editModal.item as RetourProduit;
      const updated: RetourProduit = {
        ...ret,
        quantite: editForm.quantite ?? ret.quantite,
        raison: editForm.raison,
        description: editForm.description || undefined,
        date_retour: newDatetime,
        vendeur_id: editForm.vendeur_id ?? ret.vendeur_id,
        sync_status: 'pending',
        updated_at: now,
      };
      await db.put('retours_produits', updated);
      setEditModal({ isOpen: false, type: null, item: null });
      toast.success('Retour modifié avec succès');
      onRefresh();
    } catch {
  ...
  const handleDelete = async (pin: string) => {
    ...
    try {
      const db = await getDB();
      const ret = deleteModal.item as RetourProduit;
      const key = ret.id ?? ret.local_id;
      await db.delete('retours_produits', key as any);
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-3">
          {filtered.map((ret, index) => {
            ...
            const pointeur = pointeurs.find(p => p.id === ret.pointeur_id);
            const isMine = ret.pointeur_id === currentUser.id;
            const itemKey =
              ret.id != null
                ? `ret-${ret.id}`
                : `ret-${ret.local_id ?? index}`;

            // Libellé dynamique de la raison
            const raisonLibelle = resolveLibelle(ret.raison, raisons);
            const raisonColor =
              RAISON_COLORS[ret.raison] ?? DEFAULT_RAISON_COLOR;

            return (
              <div
                key={itemKey}
                className={`card-premium p-4 transition-all ${
                  isMine ? 'ring-1 ring-primary/20' : ''
                }`}
              >
                  ...
                  <div className="flex items-start gap-3 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        ...
                        {/* Badge raison dynamique */}
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full font-medium ${raisonColor}`}
                        >
                          {raisonLibelle}
                        </span>
                      </div>
