...
export default function LoginPage() {
  ...
  // ── AUTO-LOGIN après passation d'inventaire ──────────────────────────────
  useEffect(() => {
    if (authLoading) return;

    const autoPhone = sessionStorage.getItem('autoLoginPhone');
    const autoPin   = sessionStorage.getItem('autoLoginPin');

    if (autoPhone && autoPin) {
      // Nettoyer immédiatement pour éviter une boucle
      sessionStorage.removeItem('autoLoginPhone');
      sessionStorage.removeItem('autoLoginPin');

      // Lancer la connexion automatique
      (async () => {
        setIsSubmitting(true);
        try {
          const result = await login(autoPhone, autoPin);
          if (result.success) {
            // navigate se fera via le useEffect isAuthenticated ci-dessous
          } else {
            toast.error('Connexion automatique échouée', {
              description: result.message,
            });
            // Pré-remplir le formulaire pour que le vendeur puisse se connecter manuellement
            setPhone(autoPhone);
          }
        } finally {
  ...
  if (authLoading || isSubmitting) {
    return <DivineLoader message={isSubmitting ? 'Connexion en cours...' : 'Vérification de la session...'} />;
  }
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-secondary/50 via-background to-background">
      ...
      <div className="relative z-10 p-6 text-center safe-area-bottom">
        <p className="text-xs text-muted-foreground">
          © 2026 EasyGest BP - Tous droits réservés
        </p>
