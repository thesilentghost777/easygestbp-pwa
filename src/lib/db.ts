...
// ── NOUVEAU : raisons de retour dynamiques ─────────────────────────────────
export interface RaisonRetour {
  id: number;
  code: string;    // ex: "perime", "abime", "mauvaise_qualite"
  libelle: string; // ex: "Périmé", "Mauvaise qualité"
  actif: boolean;
  sync_status: 'synced' | 'pending' | 'conflict';
  last_synced_at?: string;
  updated_at: string;
...
export interface RetourProduit {
  ...
  produit_id: number;
  quantite: number;
  raison: string; // string (plus enum figé) pour accepter les codes dynamiques
  description?: string;
  verrou: boolean;
  date_retour: string;
  sync_status: 'synced' | 'pending' | 'conflict';
...
interface EasyGestDB extends DBSchema {
  ...
  raisons_retour: {
    key: number;
    value: RaisonRetour;
    indexes: { 'by-code': string; 'by-sync': string };
  };
  receptions_pointeur: {
    key: number;
...
// ⚠️ VERSION INCRÉMENTÉE de 1 → 2 pour ajouter le store raisons_retour
const DB_VERSION = 2;
  ...
  dbInstance = await openDB<EasyGestDB>(DB_NAME, DB_VERSION, {
    upgrade(db, oldVersion) {
      ...
      if (oldVersion < 1) {
        ...
        // Réceptions
        const recStore = db.createObjectStore('receptions_pointeur', {
          keyPath: 'id',
          autoIncrement: true,
        });
        ...
        // Retours
        const retStore = db.createObjectStore('retours_produits', {
          keyPath: 'id',
          autoIncrement: true,
        });
        ...
        // Inventaires
        const invStore = db.createObjectStore('inventaires', {
          keyPath: 'id',
          autoIncrement: true,
        });
        ...
        // Inventaire détails
        const invDetailStore = db.createObjectStore('inventaire_details', {
          keyPath: 'id',
          autoIncrement: true,
        });
        ...
        // Sessions de vente
        const sessStore = db.createObjectStore('sessions_vente', {
          keyPath: 'id',
          autoIncrement: true,
        });
      ...
      // ── v2 : ajout raisons_retour ────────────────────────────────────────
      if (oldVersion < 2) {
        if (!db.objectStoreNames.contains('raisons_retour')) {
          const raisonStore = db.createObjectStore('raisons_retour', { keyPath: 'id' });
          raisonStore.createIndex('by-code', 'code', { unique: true });
          raisonStore.createIndex('by-sync', 'sync_status');
        }
      }
    },
  });
...
// ── Helper : récupérer les raisons actives (avec fallback hardcodé) ──────────
export async function getRaisonsRetour(): Promise<RaisonRetour[]> {
  try {
    const db = await getDB();
    const raisons = await db.getAll('raisons_retour');
    const actives = raisons.filter(r => r.actif);
    if (actives.length > 0) return actives;
  } catch (e) {
    console.warn('[getRaisonsRetour] Erreur IDB, fallback hardcodé', e);
  }
  // Fallback si le store est vide (premier lancement avant sync)
  return [
    { id: 1, code: 'perime',  libelle: 'Périmé',  actif: true, sync_status: 'synced', updated_at: '' },
    { id: 2, code: 'abime',   libelle: 'Abîmé',   actif: true, sync_status: 'synced', updated_at: '' },
    { id: 3, code: 'autre',   libelle: 'Autre',    actif: true, sync_status: 'synced', updated_at: '' },
  ];
}